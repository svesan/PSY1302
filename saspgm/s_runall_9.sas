...
* ............: cohort.                                                       ;
* ............:                                                               ;
* Note........: Update following reviewer comments from JAMA psych.           ;
* ............: 1. Added info about planned visits. In supplementtary analyses;
* ............:    I now only considered unplanned or inpatient visits as PPD ;
* ............:    but to define a history I keep the planned                 ;
* ............:                                                               ;
*-----------------------------------------------------------------------------;
* Data used...:                                                               ;
* Data created:                                                               ;
...
%printto(s_dm_ver9);
  ...
  %inc saspgm(s_dm11);    *-- Data management;
...
*--------------------------------------;
%printto(s_appendix_3_ver9);
  options ls=135 ps=43 notes source source2;

  %inc saspgm(s_niceprt2) /source;    
...
* --------------------                                                                        ;
* 1. Restricting follow-up to later years >= 2005                                             ;
* 2. using alternative population based date of alternative                                   ;
*---------------------------------------------------------------------------------------------;
%reset(mode=3, app=3, reset=content pageno table prefix, gpageno=N);

%printto(s_appendix_4_ver9);
  options ls=160 ps=43 notes source source2;
  %inc saspgm(s_fmt1);
...
*--------------------------------------;
%printto(s_appendix_5_ver9);
  options ls=160 ps=43 notes source source2;

  %inc saspgm(s_s1_niceprt3) /source;    

%cleanup(content=Y);


*---------------------------------------------------------------------------------------------;
* Sensitivity analysis after JAMA psych reviewer commetns                                     ;
* --------------------                                                                        ;
* 1. Only consider planned or inpatient visits as PPD. Allow any diagnosis to define history  ;
*---------------------------------------------------------------------------------------------;
%reset(mode=3, app=5, reset=content pageno table prefix, gpageno=N);

%printto(s_appendix_6_ver9);
  options ls=160 ps=43 notes source source2;

  %inc saspgm(s_poana_acute_ppd8) /source;   *-- Poisson regression PPD starting follow-up from birth;

  %inc saspgm(s_acute_niceprt2) /source;
