/***
*-- Dates for in and out-patient care;
proc sql;
  select mean(psych_dat) as average format=yymmdd10., min(psych_dat) as min format=yymmdd10., max(psych_dat) as max format=yymmdd10.
  from diag2
  group by source
  ;
run;quit;
...
data scdiag3;
  set sasperm.ana1(keep=mother_id sfinkt prodeliv bp forltyp diab);
  _type_=.;_freq_=.;
run;
*/
...
* Note........: 150623 added variable PVARD                                   ;
*-----------------------------------------------------------------------------;
...
proc format;
  ...
  *-- 2015-06-23. Added info about diagnosis following planned and unplanned visits;
  value pvrd    1='Planned visit' 2='Unplanned visit';
  value srcfmt  1='In-Patient'    2='Out-Patient';
run;
...
libname crime2 oracle dbprompt=no user=micsil pw="cd92An_yt7"
...
proc sql;
  ...
  *-- Exclude multiple births;
  ...
  select LOPNR as mother_id, input(x_date, yymmdd8.) as psych_dat length=4 format=date9. label='Date depression',
         in_nr length=4, diag_nr length=4, icd_nr length=3 label='ICD', source length=3 format=srcfmt.,
         diagnos as diagnos label='Diagnosis',
         2 - pvard as pvard length=3 label='Planned visit' format=yesno.

  from connection to oracle (
    select a.LOPNR, a.IN_NR, a.DIAG_NR, a.ICD_NR, a.diagnos, a.SOURCE, a.x_date,
           b.pvard

    from v_patient_diag a
    left join v_patient_base b
    on a.LOPNR=b.LOPNR and a.IN_NR=b.IN_NR and a.SOURCE=b.SOURCE
    where (substr(trim(a.diagnos),1,5) in ('29620','29621','29622','29623','29630','29632','29634',
                                         '29699','30110','30120','30130','648E0','648E3')) or
          (substr(trim(a.diagnos),1,4) in ('296B','301B', '648E')) or
          (substr(trim(a.diagnos),1,3) in ('309','311','F32','F33','F34','F38','F39')) or
          (substr(trim(a.diagnos),1,2) in ('F5'))
    order by a.lopnr
  );
...
*-- NOTE: For history do not take pvard into account;
data dephist
  ...
  ;
  keep mother_id dephist_dat dephist pvard;
...
*-- Note: _vard dataset for exploring the pvard data only;
proc sort data=diag2;by mother_id psych_dat;run;
data postpart1(drop=daysbirth)
     postpart1_pvard
  ;
  keep mother_id postp_dat postpartum pvard source daysbirth;
  attrib postp_dat    length=4 format=yymmdd10. label='Date of postpartum event'
         ...
         daysbirth    length=4                  label='Days between birth and diagnosis'
  ;
  set diag2(in=diag1 keep=mother_id psych_dat child_bdat pvard source);
  by mother_id psych_dat;
  daysbirth=psych_dat-child_bdat;

  *-- Delete if > 1 yr after delivery or before delivery ;
...
*-- And only considering acute, unplanned, events;
data postpart_pvard;
  drop pvard;
  rename postpartum=postpartum_acute postp_dat=postp_acute_dat;
  set postpart1(where=(pvard=0 or source=1));by mother_id postp_dat;
  if first.mother_id;
run;
...
data br2;
  ...
  merge br1 (in=br1)
        ...
        postpart_pvard(in=postpart_pvard)
        ;
  by mother_id;
  ...
  if br1 then do;
    ...
    *-- Postpartum - acute;
    if not postpart_pvard then postpartum_acute=0;
...
data ana1(where=(_mis_=0));
  ...
  attrib cens  length=3 label='Censored (Y/N)'
         ...
         exit_mo    length=5 label='Month of cohort exit'
         mage_catb  length=3 label='Maternal age'             format=mageb.
         plen_cat   length=3 label='Pregnancy length (weeks)' format=plencat.

         cens_acute    length=3 label='Censored (Y/N)'
         event_acute   length=3 label='Postpartum event (Y/N)'
         exit_acute    length=4 label='Age at cohort exit'
         exit_acute_mo length=5 label='Month of cohort exit'
  ;
  ...
  *-- 2015-06-24 added variables for acute postpartum as well;
  if postp_acute_dat>.z then do;
    exit_acute   =(postp_acute_dat - child_bdat)/365.25;
    exit_acute_mo=exit*12;
    cens_acute   =0; event_acute=1;

    *-- Bug fix 2015-01-08. In poisson regression step PPD the same day as birth were removed;
    if exit_acute_mo=0 then exit_acute_mo=1/30;
  end;
  else do;
    exit=1; cens=1; event=0; exit_mo=12;
    exit_acute=1; cens_acute=1; event_acute=0; exit_acute_mo=12;
  end;
...
proc sort data=postpart1_vard out=t1;by pvard daysbirth;run;
proc freq data=t1;
  table daysbirth / noprint out=t2 outcum;
  by pvard;
run;

proc gplot data=t2;
  where pvard>.;
  symbol1 c=red  width=2 i=join;
  symbol2 c=blue width=2 i=join;
  plot cum_pct * daysbirth = pvard;
run;quit;




proc means data=diag2 n nmiss missing;
  var psych_dat;
  class pvard source;
run;
