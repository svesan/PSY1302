*-----------------------------------------------------------------------------;
* Study.......: PSY1302                                                       ;
* Name........: s_poanasens_ppd1.sas                                           ;
* Date........: 2014-11-12                                                    ;
* Author......: svesan                                                        ;
* Purpose.....: Sensitivity analysis checking importance of calendar time     ;
* Note........: The program build on to the s_poanasens_ppd1 program from which   ;
...
proc sql;
  ...
  select mother_id, 0 as entry, exit_mo, event, cs, preg_len, deform, withfather, dephist,
         ...
         child_bdat, dephist_dat, cbyear
  from ana1
  where mage GE 15 and mage LE 49
  ;
quit;

*-- Restrict to year 2005 and onwards;
data s1(where=(mark=0) drop=_a_ _b_);
  retain _mark_ _a_ _b_ 0;
  set s0 end=eof;
  mark=0;

  *-- delete births before 2005;
  if cbyear<2005 then do;
    _a_=_a_+1; mark=1;
  end;

  *-- Recode depression history to NO if earlier than 2 years before birth;
  if cbyear GE 2005 and dephist=1 and (child_bdat - dephist_dat GT 365*2) then do;
    dephist=0; _b_=_b_+1;
  end;

  if eof then do;
    put 'NOTE: ' _a_ 'birth excluded since before 2005';
    put 'NOTE: ' _b_ 'women with depression history earlier than 2 yrs before birth were not considered';
    put '          (they are coded as no psychiatric history)';
  end;
run;
...
* Summary statistics                                  ;
*-----------------------------------------------------;
title1 'Sensitivity analysis 1 - Summary statistics: PPD analysis';
%tit(prog=s_poanasens_ppd1);
...
title1 'Sensitivity analysis 1 - PPD frequencis by exposure';
%tit(prog=s_poanasens_ppd1);
proc freq data=s1;
...
title1 'Sensitivity analysis 1 - PPD frequency and number of birth';
%tit(prog=s_poanasens_ppd1);
proc freq data=s1;
...
title1 'Sensitivity analysis 1 - PPD frequencis by time internal';
%tit(prog=s_poanasens_ppd1);
proc freq data=s3;
...
title1 'Sensitivity analysis 1 - Cumulative PPD count by time';
...
PROC REPORT DATA=s7 LS=130 PS=43  SPLIT=" " PANELS=10  PSPACE=4   HEADLINE NOCENTER MISSING nowindows;
  ...
  DEFINE  CUM_PCT  / DISPLAY FORMAT=5.1 WIDTH=5     SPACING=1   RIGHT ORDER=INTERNAL "Cum Pct" ;
RUN;QUIT;
