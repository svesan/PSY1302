...
data scdiag2;
  ...
  if br1 and scdiag1 then do;
    ...
    if substr(trim(gdiag),1,4) in ('O702','O703') then sfinkt=1;
    else if substr(trim(gdiag),1,3) in ('O63') then prodeliv=1;
    else if substr(trim(gdiag),1,4) in ('O240','O241','O242','O243','O244') then diab=1;
    else if substr(trim(gdiag),1,3) in ('O10','O11') or hyperton='1' then hyp=1;
    else if substr(trim(gdiag),1,3) in ('O14','O15') then bp=1;
  end;
  else delete;
run;
