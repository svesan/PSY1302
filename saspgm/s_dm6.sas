...
data scdiag2;
  keep mother_id sfinkt prodeliv hyp bp forltyp diab;
  ...
  if br1 and scdiag1 then do;
    sfinkt=0; prodeliv=0; prediab=0; gravdiag=0; bp=0; preeclam=0;
...
data a;
set scdiag1(keep=mdiag);
lm=length(mdiag);
if substr(trim(mdiag),1,4) in ('O244') then gravdiab=1;
run;
