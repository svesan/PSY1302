...
%tit(prog=s_poana_ppd8);
proc means data=s1 n nmiss mean stderr q1 q3 min max sum maxdec=1;
  var event mage preg_len deform cs withfather psycho sfinkt prodeliv bp cbmi;
  class dephist;
run;
...
proc freq data=s1;
  where event=1;
  table exit_mo / outcum out=t6 noprint;
  by dephist;
run;
...
proc glimmix data=s5 order=internal;
  ...
  model event = interval mage_catb cbyear_cat cs plen_cat deform withfather sfinkt forltyp prodeliv bp wperc diab
...
proc glimmix data=s5 order=internal;
  ...
  model event = interval dephist mage_catb cbyear_cat cs deform withfather psycho sfinkt forltyp prodeliv bp wperc diab preg_len
  / dist=poisson offset=logoffset link=log s htype=2;
...
proc glimmix data=s5 order=internal;
  ...
  model event = interval mage_catb cbyear_cat cs deform withfather psycho sfinkt forltyp prodeliv bp wperc diab preg_len
  / dist=poisson offset=logoffset link=log s htype=2;
...
title2 'Adjusting for interval only';
%tit(prog=s_poana_ppd8);
ods output estimates=cr_ppdest16;
proc glimmix data=s5 order=internal;
  class interval cbmi;
  model event = interval cbmi
  / dist=poisson offset=logoffset link=log s htype=2;
...
proc glimmix data=s5 order=internal;
  ...
  model event = dephist mage_catb cbyear_cat interval cbmi
  / dist=poisson offset=logoffset link=log s htype=2;
...
proc glimmix data=s5 order=internal;
  ...
  model event = mage_catb cbyear_cat interval cbmi
  / dist=poisson offset=logoffset link=log s htype=2;
...
ods output lsmeans=ppd_lsm2;
ods select lsmeans;

proc glimmix data=s5 order=internal;
  class interval cbyear_cat mage_catb dephist;

  model event = dephist mage_catb cbyear_cat interval interval*dephist
  / dist=poisson offset=logoffset link=log s htype=2;
...
data ppd_lsmx2;
  ...
  set ppd_lsm2(keep=interval mu lowermu uppermu dephist);
  mu     =exp(log(mu*10000)) ;
...
ods graphics / reset=index imagefmt=png imagename="ppd_dephist";
*title1 'Depression rate by depression history';
%tit(prog=s_poana_long3,h=0.91);
proc sgpanel data=ppd_lsmx2;
...
title1;footnote;
proc datasets lib=work mt=data nolist;
  delete s1-s7 t6 t7 v1-v15 xtmp1-xtmp5 cr_ppdest1-cr_ppdest16
         ppdest1-ppdest6 ppd_lsm2 ppd_lsmx2;
quit;
