...
*-- Only accept depression history as being diagnosed the 2 years before child birth AND only among birth from 2005;
data s1;
  set s0(where=(cbyear GE 2005));

  if dephist=1 then do;
    if dephist le .z or child_bdat le .z then abort;
    if dephist_dat < (child_bdat - (365*2)) then dephist=0;
  end;
