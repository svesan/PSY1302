...
data metadata;
  set crime2.v_metadata;
run;

data metadata_variables;
  set crime2.v_metadata_variables;
run;

proc sql;
select * from metadata where table_name in ('PATIENT_DIAG','PATIENT_BASE');


proc sort data=crime2.v_patient_base(keep=lopnr in_nr source) nodupkey out=ttt;
  by lopnr in_nr source;
run;
