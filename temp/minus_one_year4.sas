...
proc sgpanel data=pred2;
  panelby cbyear_cat / rows=1 columns=2 novarname;
...
proc glimmix data=t7 order=internal;
  class interval cbyear_cat mage_cat dephist big_int;
  model event = dephist mage_cat cbyear_cat big_int interval cbyear_cat*interval
  / dist=poisson offset=logoffset link=log s htype=2;
  estimate 'With depr. history' dephist -1 1 / exp alpha=0.05;
...
*-- Restrict to women with history of depressions;
proc glimmix data=t7 order=internal;
  where dephist=1;
  class interval cbyear_cat mage_cat;
  model event = mage_cat cbyear_cat interval cbyear_cat*interval
  / dist=poisson offset=logoffset link=log s;
...
*-- Restrict to women with no history of depressions;
proc glimmix data=t7 order=internal;
  where dephist=0;
  class interval cbyear_cat mage_cat;
  model event = mage_cat cbyear_cat interval cbyear_cat*interval
...
proc glimmix data=t7 order=internal;
  class interval;
  model event = interval
  / dist=poisson offset=logoffset link=log s;
  lsmeans interval / ilink plots;
  output out=pred1 pred(ilink)=p lcl(ilink)=lcl ucl(ilink)=ucl stderr;
run;
proc sort data=pred2;by interval;run;
data check;
merge pred2(keep=interval p cbyear_cat) lsm1(keep=interval estimate mu);
p=p/1000;
by interval;
run;

proc sort data=pred1;by interval;run;
data check;
merge pred1(keep=interval p event where=(event=.)) lsm1(keep=interval estimate mu);
by interval;
run;
